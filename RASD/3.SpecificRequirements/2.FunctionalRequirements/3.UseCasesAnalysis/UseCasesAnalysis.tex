...
\begin{center}
	\begin{longtable}{| p{.25\linewidth} | p{.75\linewidth} |}
		...
		\begin{enumerate}
			...
			\item \ic{User} checks the "Accept \ic{Terms and conditions}" checkbox
			\item \ic{User} checks the ''Accept \ic{Privacy statement}" checkbox
		...
		\begin{enumerate}
			...
			\item "Accept \ic{Terms and conditions}" checkbox is not checked
			\item "Accept \ic{Privacy statement}" checkbox is not checked
...
\begin{center}
	\begin{longtable}{| p{.25\linewidth} | p{.75\linewidth} |}
		...
		\hline
		\textbf{Name} & \textbf{U4: \ic{User} sends \ic{User report} to SafeStreets}\\ \hline
		Actors & \ic{User}, \ic{License plate recognition service}\\ \hline
		...
		\begin{enumerate}
			...
			\item SafeStreets sends \ic{Municipality} a \ic{User report notification} about the newly generated \ic{User report}
		\end{enumerate}
		\\ \hline
...
\begin{center}
	\begin{longtable}{| p{.25\linewidth} | p{.75\linewidth} |}
		...
		\hline
		\textbf{Name} & \textbf{U8a: SafeStreets generates \ic{Possible interventions}}\\ \hline
		Actors & \ic{Municipality}\\ \hline
		Entry Conditions & \ic{Municipality} grants SafeStreets access to its own \ic{Accidents} Database\\ \hline
		Events Flow & 
		\begin{enumerate}
			\item SafeStreets requests \ic{Municipality} previous Accidents data
			\item \ic{Municipality} sends SafeStreets the requested Accidents data
			\item SafeStreets crosses its own \ic{User report} data with Accidents data in order to generate \ic{Possible interventions} 
		\end{enumerate}
		\\ \hline
		Exit Condition & SafeStreets correctly generates \ic{Possible interventions}\\ \hline
		Exceptions & No Exceptions.
		\\ \hline
		\caption*{U\subs{8a}}
	\end{longtable}
...
\begin{center}
	\begin{longtable}{| p{.25\linewidth} | p{.75\linewidth} |}
		...
		\hline
		\textbf{Name} & \textbf{U8b: \ic{Municipality} visualizes \ic{Possible interventions} suggested by \ic{SafeStreets}}\\ \hline
		Actors & \ic{Municipality}\\ \hline
		Entry Conditions & \ic{Municipality} successfully logged into SafeStreets\\ \hline
		Events Flow & 
		\begin{enumerate}
			\item \ic{Municipality} presses the “Possible interventions” button
			\item \ic{Municipality} selects a particular \ic{Possible intervention} to visualize
			\item SafeStreets shows \ic{Municipality} the required \ic{Possible intervention} 
		\end{enumerate}
		\\ \hline
		Exit Condition & \ic{Municipality} correctly visualizes the suggested \ic{Possible intervention}\\ \hline
		Exceptions &
		\begin{enumerate}
			\item No \ic{Possible intervention} has been generated yet.
		\end{enumerate}
		\ic{Municipality} is asked to retry later.
		\\ \hline
		\caption*{U\subs{8b}}
	\end{longtable}
\end{center}
