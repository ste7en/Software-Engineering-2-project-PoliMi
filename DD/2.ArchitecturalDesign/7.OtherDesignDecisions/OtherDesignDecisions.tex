...
\ic{User reports} constitutes sensitive data because they contain license plates numbers, GPS data and, in general, information about traffic violations that will be shown to a \ic{Municipality}. To ensure that these information won't be altered, a digital signature mechanism has been implemented to realize the chain of custody. \\

In particular, when a \ic{User} sends his \ic{User report} to the Application Server, a hash function on its data and the \ic{User} entity is computed and stored in the same Database of the report. At retrieval time, the same function is computed and the values are checked to ensure that no alterations have occurred.
